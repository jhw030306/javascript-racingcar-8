import { RacingError } from "./RacingError.js";

const Validator = {
  validateCars(cars) {
    if (cars.some((x) => x.length > 5)) {
      throw new RacingError(
        "자동차 이름은 5자 초과입니다."
      );
    }
    ...
    if (nameSet.size != cars.length) {
      throw new RacingError("자동차 이름이 중복입니다.");
    }

    if (cars.some((x) => x.trim() === "")) {
      throw new RacingError("자동차 이름이 공백입니다.");
    }
  ...
  validateAttemptCount(attemptCount) {
    if (isNaN(attemptCount)) {
      throw new RacingError("횟수가 숫자가 아닙니다.");
    }

    if (attemptCount.trim() === "") {
      throw new RacingError("횟수는 공백일 수 없습니다.");
    }

    const count = Number(attemptCount);

    if (count < 0) {
      throw new RacingError("횟수가 음수입니다.");
    }
    ...
    if (!Number.isInteger(count)) {
      throw new RacingError("횟수가 소수입니다.");
    }

    if (count > 1000) {
      throw new RacingError("횟수가 너무 큽니다.");
    }
  },
};
